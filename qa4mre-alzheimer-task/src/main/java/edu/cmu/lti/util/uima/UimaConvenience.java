...
public class UimaConvenience extends BasicConvenience {
	...
	/**
	 * Takes a Java list containing UIMA FeatureStructures, and creates an
	 * FSArray
	 * 
	 * @param list
	 ...
	 *            whether to include a converted form of filepath in document
	 *            name.
	 * @return
	 */
	public static String getDocId(JCas aJCas, boolean appendOffset, boolean includeFullUri) {
		...
		AnnotationIndex<Annotation> docIndex = aJCas.getJFSIndexRepository().getAnnotationIndex(SourceDocumentInformation.typeIndexID);
	...
	/**
	 * Find the "real" _InitialView Can't simply get it by name as it may have
	 * been sofa mapped to another view
	 * 
	 * @param jcas
	...
	public static String getDocumentDate(JCas aJCas) {
		FSIndex docIndex = aJCas.getJFSIndexRepository().getAnnotationIndex(SourceDocumentInformation.typeIndexID);
	...
	/**
	 * Check whether or not a given CAS has a SofA defined. This method uses an
	 * efficient (i.e., exception free) approach that relies heavily in
	 * UIMA-internals. It should be changed when this functionality is available
	 * inside UIMA.
	 * 
	 * @param cas
	...
	public static boolean isInvalid(Annotation a, JCas jcas) {
		return (a.getBegin() >= a.getEnd() || (a.getBegin() < 0) || (a.getEnd() > jcas.getDocumentText().length()));
	}

	/**
	 * Identifies the number of annotations of a given type in the specified
	 * JCas.
	 * 
	 * @param aJCas
	 * @param clazz
	 ...
	 */
	public static int numberOfAnnotations(final JCas aJCas, final Class clazz) throws CASRuntimeException {
		try {
	...
	/**
	 * Moves an FSIterator so it is just before the given target span. That is,
	 * the next annotation returned from i.next() (or i.get()) will be the first
	 * one in the CAS with <code>begin >= targetSpan.begin</code> and
	 * <code>end &lt;= targetSpan.end</code>.
	 * 
	 * @param i
	 *            an iterator
	 * @param targetSpan
	 *            annotation representing the target span to move the iterator
	 *            before
	 */
	public static void moveBeforeSpan(FSIterator i, AnnotationFS targetSpan) {
		...
		while (true) {
			...
			if (annot.getBegin() < targetSpan.getBegin() || (annot.getBegin() == targetSpan.getBegin() && annot.getEnd() > targetSpan.getEnd())) {
				i.moveToNext();
				return;
			}
			i.moveToPrevious();
	...
	/**
	 * Returns a list of annotations of the specified type in the specified CAS.
	 * The difference of this with JCasUtil.select is that this method will
	 * store everything into a list first, so if modifying stuff, it won't raise
	 ...
	 * @return
	 */
	public static <T extends TOP> FSList appendFSList(JCas aJCas, FSList oldFSList, T newItem, Class<T> clazz) {
		List<T> newList;
		if (oldFSList != null) {
	...
	public static <T extends TOP> FSList replaceFSList(JCas aJCas, FSList oldFSList, T oldItem, T newItem, Class<T> clazz) {
	 ...
	 * @return
	 */
	public static <T extends TOP> FSList appendAllFSList(JCas aJCas, FSList oldFSList, Collection<T> newItems, Class<T> clazz) {
		List<T> newList;
	...
	public static <T extends TOP> FSList mergeFSList(JCas aJCas, FSList fsList1, FSList fsList2, Class<T> clazz) {
		if (fsList2 != null) {
	 ...
	 * @return
	 */
	public static <T extends TOP> FSList removeFromFSList(JCas aJCas, FSList oldFSList, T itemToRemove, Class<T> clazz) {
		List<T> newList;
		if (oldFSList != null) {
			newList = new ArrayList<T>();
	...
	/**
	 * Returns the annotation with the same range as the specified target
	 * annotation.
	 * 
	 * @param aJCas
	 * @param annotationList
	 * @param targetAnnotation
	 * @return the annotation with the same range as the specified target
	 *         annotation
	 */
	public static <T, U extends Annotation> T getAnnotationWithSameRange(JCas aJCas, List<T> annotationList, U targetAnnotation) {
	...
	public static <T extends TOP, U extends Annotation> T getAnnotationWithMinRange(JCas aJCas, Class<T> clazz, U targetAnnotation) {
	...
	/**
	 * Returns the annotation that surrounds the target annotation with the
	 * minimum range.
	 * 
	 * @param aJCas
	 * @param annotationList
	 * @param targetAnnotation
	 * @return the annotation that surrounds the target annotation with the
	 *         minimum range
	 */
	public static <T, U extends Annotation> T getAnnotationWithMinRange(JCas aJCas, List<T> annotationList, U targetAnnotation) {
		if (annotationList == null || annotationList.isEmpty()) {
			return null;
		}
	...
	/**
	 * Returns the annotation of the specified type which is closest to the
	 * specified annotation within the specified region.
	 * 
	 * @param aJCas
	 * @param targetAnnotation
	 * @param type
	 * @param begin
	 * @param end
	 * @return the annotation of the specified type which is closest to the
	 *         specified annotation within the specified region
	 */
	public static <T, U extends Annotation> U getClosestAnnotation(JCas aJCas, T targetAnnotation, Class<U> type, int begin, int end) {
	...
	/**
	 * Returns true if the specified annotation covers a valid region of text;
	 * false otherwise.
	 * 
	 * @param annotation
	 * @return true if the specified annotation covers a valid region of text;
	 *         false otherwise
	 */
	 ...
	 * @return
	 */
	public static <T extends Annotation> T selectCoveredFirst(JCas aJCas, Annotation anno, Class<T> clazz) {
	 ...
	 * @return
	 */
	public static <T extends Annotation> T selectCoveredFirst(JCas aJCas, int begin, int end, Class<T> clazz) {
	...
	public static <K extends Annotation, T extends Annotation> Map<K, Collection<T>> indexCoveredAndCovering(JCas aJCas, Class<K> keyType,
			Class<T> valueType) {
	 ...
	 * @param ann1
	 * @param ann2
	 * @return True if the specified two annotations are the same; false
	 *         otherwise.
	 */
	...
	public static String getShortDocumentName(JCas aJCas) {
		SourceDocumentInformation srcDocInfo = JCasUtil.selectSingle(aJCas, SourceDocumentInformation.class);
		if (srcDocInfo == null) {
			return null;
		}
